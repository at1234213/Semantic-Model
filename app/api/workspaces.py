"""Workspace routes. Every route is scoped to the authenticated tenant."""
...
def create_workspace(
    ...
    if tenants_service.get(db, principal.tenant_id) is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Tenant not found")

    try:
        workspace = workspaces_service.create(db, tenant_id=principal.tenant_id, name=payload.name)
...
def list_workspaces(
    ...
    return workspaces_service.list_for_tenant(
...
def get_workspace(
    ...
    workspace = workspaces_service.get(db, tenant_id=principal.tenant_id, workspace_id=workspace_id)
